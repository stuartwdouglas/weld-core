...
public class InstanceImpl<T> extends AbstractFacade<T, Instance<T>> implements Instance<T>, Serializable
{
   ...
   public T get()
   {      
      Bean<?> bean = getBeanManager().getBean(new ResolvableBuilder(getBeanManager().getContextId(), getType()).addQualifiers(getQualifiers()).setDeclaringBean(getInjectionPoint().getBean()).create());
